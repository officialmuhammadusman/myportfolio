      ...
      <div className="layout-wrap relative z-10">
        ...
        <div className="sm:hidden -mx-4 px-4 snap-scroll-x pb-8">
          {SELECTED_PROJECTS_DATA.map((project, index) => (
            ...
            >
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#FF6A00] mb-2">
                  {project.subtitle}
                </p>
                <h3 className="font-display text-2xl tracking-tight text-[#FFF7ED] mb-4">
                  {project.title}
                </h3>
              ...
              {project.link && (
                <a
                  ...
                  className="mt-6 flex items-center justify-between w-full rounded-xl bg-white/[0.03] border border-white/[0.08] p-3 text-xs font-semibold uppercase tracking-[0.1em] text-[#FFF7ED]/80 hover:bg-white/[0.08] transition-colors"
                >
                  View Live Site
        ...
        <div className="hidden sm:grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-6xl mx-auto">
          {SELECTED_PROJECTS_DATA.map((project, index) => (
            <motion.div
              ...
              style={{ opacity: 1 }}
              className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8 transition-colors hover:bg-white/[0.04] hover:border-[#FF6A00]/20"
            >
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  {project.link && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="h-8 w-8 rounded-full bg-white/[0.05] flex items-center justify-center shrink-0 border border-white/[0.05] hover:bg-[#FF6A00] transition-colors group/btn"
                    >
                      <ArrowUpRight size={16} className="text-[#FFF7ED]/50 group-hover/btn:text-[#0A0A0A]" />
                    </a>
                  )}
                </div>
                
                <h3 className="font-display text-[2rem] tracking-tight text-[#FFF7ED] mb-5">
                  {project.title}
                </h3>
